...
from libs.processing.multi_category_encoder import MultiCategoryEncoder, MultiCategoryEncoderModel

from libs.processing.imp.onehot import OneHotProcessing
from libs.processing.imp.number import IntProcessing
from libs.processing.imp.number import DoubleProcessing
from libs.processing.imp.multi_value_category import MultiValueCategoryProcessing
from libs.processing.imp.vector import VectorProcessing
from libs.processing.udfs import vector_indices,vector_values
...
def get_result(df,lable_col):
    ...
    df = df.withColumn('feature_indices', vector_indices('imp'))
    df = df.withColumn('feature_values', vector_values('imp'))
...
def processing(train, test, processing_conf):
    ...
    for processing_col in processing_col_conf:
        processing_name = processing_col['imp']
