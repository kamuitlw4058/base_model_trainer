...
def get_ctr_feature():
    sql_temp ="""
            (select
                ...
                sum(IMP) as a{account}_{vendor}_last14_imp,
                sum(CLK) as a{account}_{vendor}_last14_clk,
                case
                    when a{account}_{vendor}_last14_imp < a{account}_{vendor}_last14_clk then 1.0
                    when a{account}_{vendor}_last14_imp >= a{account}_{vendor}_last14_clk then floor(a{account}_{vendor}_last14_clk/a{account}_{vendor}_last14_imp*1.0,1)
                    else null
                end as a{account}_{vendor}_last14_ctr,
            ...
            from
                (
                    ...
                    from
                        zampda.rtb_all
                    prewhere
                        EventDate >= toDate('{target_day:%Y-%m-%d}')-14 and TotalErrorCode=0
                    where
